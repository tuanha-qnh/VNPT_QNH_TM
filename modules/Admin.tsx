...
const Admin: React.FC<AdminProps> = ({ units, users, currentUser, onRefresh }) => {
  ...
  const handleOpenAddModal = () => {
    if (activeTab === 'users') {
      ...
      setEditingItem(null);
      setFormData({ name: '', code: `QNH${Math.floor(Math.random() * 900) + 100}`, parentId: rootUnit?.id || null, level: rootUnit ? rootUnit.level + 1 : 1, includeInPtmReport: false, includeInBroadbandReport: false });
    }
    setIsModalOpen(true);
  };
