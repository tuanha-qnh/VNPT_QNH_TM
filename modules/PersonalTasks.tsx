...
import { Plus, Trash2, Edit2, CheckCircle2, Circle, Clock, MoreVertical, X, Save, Search, Calendar, StickyNote, Loader2 } from 'lucide-react';
...
const PersonalTasks: React.FC<PersonalTasksProps> = ({ currentUser }) => {
  ...
  const fetchTasks = async () => {
    setIsLoading(true);
    try {
      // Vì firebaseClient hiện tại getAll lấy hết, ta sẽ lọc tay theo userId
  ...
  const filteredList = tasks.filter(t => t.name.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="flex justify-between items-center border-b pb-6">
        <div>
          <h2 className="text-3xl font-black text-slate-800 tracking-tighter flex items-center gap-3 uppercase">
            <StickyNote className="text-amber-500" size={36}/> Sổ tay công việc cá nhân
          </h2>
          <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mt-1">To-do list của riêng bạn</p>
        </div>
      ...
      <div className="bg-white rounded-[40px] shadow-sm border overflow-hidden">
        <div className="p-8 border-b bg-slate-50/50">
          <div className="relative">
            ...
            <input placeholder="Tìm kiếm đầu việc cá nhân..." className="w-full pl-12 pr-6 py-3.5 bg-white border-2 rounded-[20px] text-sm font-bold focus:border-amber-500 outline-none transition-all" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
        </div>

        <div className="divide-y">
          {isLoading ? (
            <div className="p-20 text-center"><Loader2 className="animate-spin mx-auto text-amber-500" size={40}/></div>
          ) : filteredList.length === 0 ? (
            <div className="p-20 text-center text-slate-400 font-bold italic">Bạn chưa có ghi chú công việc nào.</div>
          ) : filteredList.map(task => (
            <div key={task.id} className="p-6 hover:bg-amber-50/30 transition-all group flex items-start gap-4">
              <button 
                onClick={async () => {
                ...
                className={`mt-1 transition-colors ${task.status === 'Đã hoàn thành' ? 'text-green-600' : 'text-slate-300 hover:text-amber-500'}`}
              >
              ...
              <div className="flex-1 space-y-2">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className={`text-base font-black text-slate-800 ${task.status === 'Đã hoàn thành' ? 'line-through opacity-50' : ''}`}>{task.name}</h3>
                    <div className="flex items-center gap-4 mt-1">
                      <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase ${statusColors[task.status]}`}>{task.status}</span>
                      <span className="text-[10px] font-bold text-red-500 flex items-center gap-1"><Clock size={12}/> Hạn: {task.deadline}</span>
                    </div>
                  </div>
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => { setFormData(task); setShowForm(true); }} className="p-2 hover:bg-white rounded-xl text-slate-400 hover:text-blue-600 shadow-sm border"><Edit2 size={16}/></button>
                    <button onClick={() => handleDelete(task.id)} className="p-2 hover:bg-white rounded-xl text-slate-400 hover:text-red-500 shadow-sm border"><Trash2 size={16}/></button>
                  </div>
                </div>
                {task.content && <p className="text-xs text-slate-500 font-medium leading-relaxed">{task.content}</p>}
                {task.note && <div className="text-[10px] bg-amber-50 text-amber-700 px-3 py-1.5 rounded-lg border border-amber-100 font-bold">Ghi chú: {task.note}</div>}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="fixed inset-0 z-[120] bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-[40px] w-full max-w-xl shadow-2xl overflow-hidden border animate-zoom-in">
            <div className="p-8 border-b bg-slate-50 flex justify-between items-center">
              <h3 className="text-lg font-black text-slate-800 uppercase">{formData.id ? 'SỬA GHI CHÚ' : 'THÊM VIỆC CẦN LÀM'}</h3>
