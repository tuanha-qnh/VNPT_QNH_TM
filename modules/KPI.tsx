...
const KPI: React.FC<KPIProps> = ({ users, units, currentUser, mode, kpiDefinitions, onRefresh }) => {
  ...
  const handleReadData = async () => {
    ...
    try {
      let finalUrl = dsConfig.url.trim(); if (finalUrl.includes('/edit')) finalUrl = finalUrl.split('/edit')[0] + '/export?format=csv';
      const res = await fetch(finalUrl); if (!res.ok) throw new Error("Không thể tải file.");
      const csv = await res.text(); const wb = XLSX.read(csv, { type: 'string' });
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (rowsToProcess.length === 0) {
        const res = await fetch(dsConfig.url); const csv = await res.text(); const wb = XLSX.read(csv, { type: 'string' });
      ...
      fetchKpis(); alert(`Đã import thành công ${rowsToProcess.length} dòng KPI.`);
    } catch (e) { alert("Lỗi khi import."); } finally { setIsSyncing(false); }
  };
